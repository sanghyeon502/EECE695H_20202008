...
" User input value "
TOTAL = 27000  # total step of training
...
def Test_phase(model, args, k):
    ...
    for i in range(TEST_SIZE):
        for episode in test_loader:
            ...
            """
            output_query = model(data_query.cuda())
            output_shot = model(data_shot.cuda())
            logits = torch.zeros(data_query.size(0), args.nway).cuda()
            for j in range(args.nway):
                logits[:,j] = torch.norm(output_query - torch.mean(output_shot[j * args.kshot: (j + 1) * args.kshot], 0),dim=1)
            pred = torch.argmin(logits, dim=1)
...
def train(args):
    ...
    for i in range(TOTAL):
        for episode in data_loader:
            ...
            for j in range(args.nway):
                logits[:,j] = torch.norm(output_query - torch.mean(output_shot[j * args.kshot: (j + 1) * args.kshot], 0),dim=1)
            loss = criterion(-1*logits, labels)
            """ TODO 2 END """
